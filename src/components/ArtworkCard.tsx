...
export default function ArtworkCard({ artwork, index }: Props) {
  ...
  const handleLightboxTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(diff) > 50) {
      if (diff > 0 && lightboxIndex < (artwork.previewImages?.length || 0) - 1) {
        scrollToLightboxPage(lightboxIndex + 1);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        ...
        if (lightboxIndex < (artwork.previewImages?.length || 0) - 1) {
          scrollToLightboxPage(lightboxIndex + 1);
        }
  ...
  }, [isLightboxOpen, lightboxIndex, artwork.previewImages?.length]);
        ...
        >
          ...
          <div className="min-w-full h-full snap-center relative flex items-center justify-center bg-[#0c1611] overflow-hidden">
                    ...
                    if (audioRef.current) {
                      if (!nextMuted) {
                        if (audioRef.current.volume === 0) {
                           // Ensure volume starts at 0 for fade in if it was previously muted
                           // but actually fadeAudio handles starting from current volume.
                        ...
                        fadeAudio(audioRef.current, 1, 500);
                      } else {
                        fadeAudio(audioRef.current, 0, 500);
                      }
                    }
                  }}
                  className="w-10 h-10 rounded-full bg-black/60 backdrop-blur-md flex items-center justify-center text-white/90 hover:bg-black/80 transition-all ring-1 ring-white/20 shadow-[0_0_15px_rgba(0,0,0,0.5)] active:scale-95 cursor-pointer touch-none"
                  title={isAudioMuted ? "Play Music" : "Mute Music"}
                >
                  {isAudioMuted ? (
                    <VolumeX className="w-5 h-5 text-white/70" />
                  ) : (
                    <Volume2 className="w-5 h-5 text-[#A5D6A7]" />
                  )}
                </button>
              </div>
            )}
            <div className="absolute inset-x-0 bottom-0 p-5 pt-16 flex flex-col gap-1 pointer-events-none bg-gradient-to-t from-black/80 via-black/40 to-transparent text-shadow-sm pb-10 z-[2]">
              <h3 className="font-serif text-white text-xl md:text-2xl drop-shadow-md leading-snug">
                {language === "zh"
                  ? artwork.title
              ...
              {language === "zh"
                ? artwork.titleEn && (
                    <span className="font-sans text-neutral-300 text-xs tracking-wide drop-shadow-md block mb-1">
                      {artwork.titleEn}
                    </span>
                  )
                : artwork.title && (
              ...
              <span className="font-sans text-xs text-[#A5D6A7] tracking-wider drop-shadow-md">
                {language === "zh"
                  ? artwork.category
                  : artwork.categoryEn || artwork.category}
              </span>
            </div>
          </div>
          ...
          <div className="min-w-full h-full snap-center bg-[#0c1611] flex flex-col items-center justify-center p-5 md:p-8 overflow-y-auto">
            <div className="w-full flex-1 flex flex-col pt-2 pb-8 items-start">
              ...
              <h2 className="text-xl font-serif text-[#E8F0EB] leading-tight shrink-0 mb-3 text-left w-full">
                ...
                {language === "zh"
                  ? artwork.titleEn && (
                      <span className="block text-sm text-neutral-500 font-sans mt-1.5">
                        {artwork.titleEn}
                      </span>
                    )
                  : artwork.title && (
              ...
              {(artwork.link ||
                artwork.link18Plus ||
                (artwork.previewImages &&
                  artwork.previewImages.length > 0)) && (
          ...
          {artwork.extraPages?.map((page, idx) => (
            <div
              key={`extra-${idx}`}
              className="min-w-full h-full snap-center relative flex flex-col items-center justify-center bg-[#0c1611] overflow-hidden px-2 pt-2 pb-5"
            >
              {page.iframeUrl ? (
                <>
                ...
                >
                  ...
                  <div className="flex items-center justify-between p-4 md:p-6 text-white absolute top-0 left-0 right-0 z-10 bg-gradient-to-b from-black/80 to-transparent">
                    <span className="font-serif tracking-widest text-sm md:text-base">
                      {language === "zh" ? "作品局部預覽" : "Preview"} (
                      {lightboxIndex + 1}/{artwork.previewImages.length})
                    </span>
                    <button
                  ...
                  <div
                    ...
                    className="flex-1 w-full h-full flex overflow-x-hidden hide-scrollbar"
                  ...
                  >
                    {artwork.previewImages.map((img, idx) => (
                      <div
                        key={idx}
                        className="min-w-full h-full snap-center flex items-center justify-center p-4 md:p-8 pt-20 pb-20 relative"
                      >
                        {!loadedImages[idx] && (
